	...
	angular.module('joinApp', []).controller('joinController',
			[ '$scope', function($scope) {
				...
				$scope.checkFinal = function() {
					...
					// 기본 주소 폼점검
					// ex) 경기 성남시 분당구 경부고속도로 409 (삼평동)
					var regex = /^[a-zA-Z0-9 | 가-힣 | / | - |  (  |  ) | ,]{2,100}$/; // 기본주소 regex(정규표현식) (교정)
					
					//alert("기본 주소 값 : "+$('#memberAddressBasic').val());
					//alert("플래그 : "+regex.test($('#memberAddressBasic').val()));
...
<body ng-app="joinApp" ng-controller="joinController">
	...
	<form id="joinForm" name="joinForm" action="../member/memberJoinProc.do"
		method="post">
		...
		<table class="table tbl_width">
			...
			<tr>
				...
				<td><input type="text" id="memberZip" name="memberZip" readonly
					...
					onClick="road_Postcode()"><br>
					</td>
			</tr>
			<tr>
				<td colspan="2" id="memberZipErrMsg"
					ng-show="joinForm.memberZip.$invalid"
					ng-model="memberZipErrMsg">우편번호를 입력하세요</td>
			</tr>
			<!-- 주소 -->
			<tr>
				<td>주소</td>
				<td><input type="text" id="memberAddressBasic"
					...
					size="70"><br>
					<!-- valid : {{joinForm.memberAddressBasic.$valid}}<br>
					invalid : {{joinForm.memberAddressBasic.$invalid}}<br> -->
					</td>
			</tr>
			<tr>
